...
def transform(file_idx, line):
    ...
    columns = annotate_filter_values(file_idx, columns)
    columns = strip_fixed_columns(file_idx, columns)

    # Add additional transformers here, each should be a function that receives a (file index, columns) argument pair
    # and returns the transformed columns

    line = join_columns(columns)
